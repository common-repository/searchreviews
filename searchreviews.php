...
function searchreviews_link($content){
	...
	if(get_post_meta($post->ID, 'sr_tags', true) != ''){
		$link = '<a href="http://searchreviews.com/search.jsp?reviews=' . get_post_meta($post->ID, 'sr_tags', true) . '" title="Reviews for ' . get_post_meta($post->ID, 'sr_tags', true) . '" class="searchReviewsLink" keywords="' . get_post_meta($post->ID, 'sr_tags', true) . '">' . get_post_meta($post->ID, 'sr_tags', true) . ' reviews</a>';
	}
	elseif(get_option('sr_showAlways') == "always"){
		$link = '<a href="http://searchreviews.com/search.jsp?reviews=' . get_the_title($post->ID) . '" title="Reviews for ' . get_the_title($post->ID) . '" class="searchReviewsLink" keywords="' . get_the_title($post->ID) . '">' . get_the_title($post->ID) . ' reviews</a>';
	}
	else{
		$link = '';
	}
